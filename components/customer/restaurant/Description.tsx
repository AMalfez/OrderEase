...
const Description = ({ slug, userId }: any) => {
  ...
  const getRestaurant = async()=>{
    try {
      const rest = await getRestaurantByRestaurantId(slug);
      setRestaurant(rest);
    } catch (error:any) {
      alert(error)
    }
  ...
  const getTestimonials = async()=>{
    try {
      const data = await getTestimonialsByRestId(slug);
  ...
  return (
    <section className="text-gray-600 body-font overflow-hidden">
          ...
          <div className="lg:w-1/2 w-full lg:pl-10 lg:py-6 mt-6 lg:mt-0">
            ...
            <div className="flex mb-4">
              <span className="flex items-center">
                ...
                <span className="text-gray-600 ml-3">{testimonials.length+""} Reviews</span>
              </span>
      ...
      {testimonials && (<Review testimonials={testimonials} />)}
    </section>
  );
};
